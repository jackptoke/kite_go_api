...
func ValidatePassword(v *validator.Validator, password string) {
	...
	v.Check(len(password) >= 8, "password", "must not be more than 8 characters")
...
func ValidateUser(v *validator.Validator, u *User) {
	...
	v.Check(len(u.Name) <= 500, "name", "must not be more than 500 characters long")
...
func (m UserModel) Insert(user *User) error {
	...
	args := []any{user.Name, user.Email, user.Activated, user.Password.hash}
...
func (m UserModel) GetByEmail(email string) (*User, error) {
	query := `
		SELECT id, created_at, name, email, activated, password_hash, version 
...
func (m UserModel) GetForToken(tokenScope, tokenPlainText string) (*User, error) {
	...
	query := `
		SELECT users.id, users.created_at, users.name, users.email, 
		       users.activated, users.password_hash, users.version 
		FROM users 
